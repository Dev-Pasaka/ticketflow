...
class ClearAllNotificationUseCase(
    ...
    suspend fun clearAllNotification(activity: Activity): Flow<Resource<List<Notification>>> = flow {
        ...
        try {
            ...
            val response = notificationRepository.clearAllNotifications(token).toNotificationList()
            emit(Resource.Success(data = response, message = "Notifications cleared successfully"))
        }
